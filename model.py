...
def sasrec(item_num: int, seq_max_len: int, num_blocks: int = 2, embedding_dim: int = 100, attention_dim: int = 100,
    ...
    seq_emb = tf.reshape(seq_attention, [tf.shape(input_seq)[0] * seq_max_len, embedding_dim]) # (b*s, d)
    # shared item embedding (for candidate item)
    candidate_emb = item_embedding_layer(candidate)  # (b, s, d)
    candidate_emb = tf.reshape(candidate_emb, [tf.shape(input_seq)[0] * seq_max_len, embedding_dim]) # (b*s, d)
